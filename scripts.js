...
function formatTimeCount(duration) {
  const seconds = Math.floor((duration / 1000) % 60);
  const minutes = Math.floor((duration / (1000 * 60)) % 60);
...
document.addEventListener("DOMContentLoaded", () => {
  const count_hoursUnit = document.getElementById("countdown_hours");
  const count_minutesUnit = document.getElementById("countdown_minutes");
  const count_secondsUnit = document.getElementById("countdown_seconds");
  ...
  function count_updateStopwatch() {
    ...
    updateFlipUnit(count_minutesUnit, formattedTimeCount.minutes, 2); // Minutes field with 2 digits
    updateFlipUnit(count_secondsUnit, formattedTimeCount.seconds, 2); // Seconds field with 2 digits
  }

  count_updateStopwatch();
  setInterval(count_updateStopwatch, 1000);
});